...
if [[ $HOST = "i686-w64-mingw32" ]]; then
  ARCHIVE_NAME="winge-windows-32bit.zip"
elif [[ $HOST = "x86_64-w64-mingw32" ]]; then
    ARCHIVE_NAME="winge-windows-64bit.zip"
    ...
    ARCHIVE_NAME="winge-linux-x64.tar.gz"
    ARCHIVE_CMD="tar -czf"
...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i Winged-network.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/winged-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/winged-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/wingedd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/wingedd.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/winged-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/winged-cli.exe $OUTDIR/bin/
