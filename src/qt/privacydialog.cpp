                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zWINGE ought to be enough for anybody." - Bill Gates, 2017
    ui->zWINGEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzWINGESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzWINGE->setEnabled(false);
        ui->pushButtonMintzWINGE->setToolTip(tr("zWINGE is currently disabled due to maintenance."));

        ui->pushButtonSpendzWINGE->setEnabled(false);
        ui->pushButtonSpendzWINGE->setToolTip(tr("zWINGE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zWINGEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzWINGE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWINGE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zWINGE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zWINGE in ") +
...
void PrivacyDialog::on_pushButtonSpendzWINGE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWINGE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zWINGE
        sendzWINGE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zWINGE
    sendzWINGE();
}

void PrivacyDialog::on_pushButtonZWingeControl_clicked()
{
    ZWingeControlDialog* zWingeControl = new ZWingeControlDialog(this);
    zWingeControl->setModel(walletModel);
    zWingeControl->exec();
}

void PrivacyDialog::setZWingeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzWingeSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzWINGE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Winged Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zWINGEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zWINGE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " WINGE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zWINGE</b>";
    ...
    // use mints from zWinge selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZWingeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZWingeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zwinge selector in case it was used
    ZWingeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zWinge Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zWINGE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zWINGE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zWINGE "));
    ui->labelzWINGEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzWINGESyncStatus->setVisible(fShow);
}
