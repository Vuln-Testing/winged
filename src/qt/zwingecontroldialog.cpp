...
#include "ui_zwingecontroldialog.h"
...
std::list<std::string> ZWingeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZWingeControlDialog::listMints;

ZWingeControlDialog::ZWingeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZWingeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZWingeControlDialog::~ZWingeControlDialog()
{
    delete ui;
}

void ZWingeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWingeControlDialog::updateList()
{
...
void ZWingeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWingeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWinge_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWingeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWingeControlDialog::GetSelectedMints()
{
...
void ZWingeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
