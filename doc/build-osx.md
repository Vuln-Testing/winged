...
====================================
This guide will show you how to build wingedd (headless client) for OSX.
...
4.  (Optional) You can also install wingedd to your path:
...
Set the **Build directory** path to the `root` of the clone Winged project. Qt Creator needs this to locate the `Makefile`.
...
To start debugging, locate the following function in `winged.cpp` and set a breakpoint.
...
------------------------
You can ignore this section if you are building `wingedd` for your own use.

`wingedd/winged-cli` binaries are not included in the `winged-Qt.app` bundle.

If you are building `wingedd` or `winged-qt` for others, your build machine should be set up
...
Once dependencies are compiled, see release-process.md for how the Winged-Qt.app
bundle is packaged and signed to create the .dmg disk image that is distributed.

Running
-------

It's now available at `./wingedd`, provided that you are still in the `src`
directory. We have to first create the RPC configuration file, though.

Run `./wingedd` to get the filename where it should be put, or just try these
commands:

    echo -e "rpcuser=wingedrpc\nrpcpassword=$(xxd -l 16 -p /dev/urandom)" > "/Users/${USER}/Library/Application Support/Winged/winged.conf"
    chmod 600 "/Users/${USER}/Library/Application Support/Winged/winged.conf"

The next time you run it, it will start downloading the blockchain, but it won't
output anything while it's doing this. This process may take several hours;
you can monitor its process by looking at the debug.log file, like this:

    tail -f $HOME/Library/Application\ Support/Winged/debug.log

Other commands:
-------

    ./wingedd -daemon # to start the winged daemon.
    ./winged-cli --help  # for a list of command-line options.
    ./winged-cli help    # When the daemon is running, to get a list of RPC commands
